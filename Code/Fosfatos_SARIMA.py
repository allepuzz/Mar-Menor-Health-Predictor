...
plt.plot(y, label='Fosfatos Real', color='purple')
plt.plot(y_pred.predicted_mean, label='Fosfatos Predicha', color='gold', linestyle='--')
plt.fill_between(y_pred_ci.index, y_pred_ci.iloc[:, 0], y_pred_ci.iloc[:, 1], color='blue', alpha=0.3)

plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.gca().xaxis.set_major_locator(mdates.DayLocator(interval=30))
plt.gcf().autofmt_xdate()

plt.title('Comparación de Valores Reales vs. Predichos de Fosfatos (SARIMA)')
plt.xlabel('Fecha')
plt.ylabel('Fosfatos')
plt.legend()
plt.grid(True)
...
print(f"MSE para Fosfatosfila (SARIMA): {rmse}")
print(f"MAE para Fosfatosfila (SARIMA): {mae}")
print(f"R² para Fosfatosfila (SARIMA): {r2}")
