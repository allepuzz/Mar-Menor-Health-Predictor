...
decomposition = seasonal_decompose(Base['Clorofila'], model='additive', period=30)  # Ajusta el período según tus datos

# Graficar los componentes
plt.figure(figsize=(15, 10))
decomposition.plot()
plt.suptitle('Descomposición de la serie temporal de Clorofila', fontsize=16)
plt.savefig('./descomposicion_clorofila.svg', format='svg')
plt.show()

# Variables para predicción
X = Base.drop(['Clorofila'], axis=1)  # Incluir Día, Mes, Año
y = Base['Clorofila']
...
print(f"MSE por split para Clorofila: {mse_scores}")
print(f"MSE medio para Clorofila: {np.mean(mse_scores)}")

# Crear un DataFrame para visualizar los resultados
results = pd.DataFrame({'Fecha': fechasall, 
                        'Clorofila_Real': real, 'Clorofila_Predicho': predict})
...
# Graficar resultados
plot_results('Clorofila', 'Clorofila_Real', 'Clorofila_Predicho', umbral, 'blue', 'red', 'cyan')
...
plt.savefig('./Clorofila_importancia.svg', format='svg')
plt.show()
