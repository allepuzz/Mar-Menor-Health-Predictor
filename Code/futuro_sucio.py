...
# Preparar los datos
n_lags = 12
df_supervised = create_supervised_data(Base['Clorofila'].values, n_lags)
...
model = RandomForestRegressor(n_estimators=100, max_depth=20, min_samples_split=2, min_samples_leaf=2,max_features='log2', bootstrap=True)
...
for i in range(18):  # Predicciones para 18 meses hasta mediados de 2025
...
fechas_predicciones = pd.date_range(start=Base.index.max(), periods=18, freq='ME')

plt.figure(figsize=(14, 7))

# Graficar los valores reales y las predicciones para el periodo histórico
plt.plot(dates, true_values, label='Valores Reales de Clorofila', color='blue')
plt.plot(dates, predictions, label='Predicciones del Modelo (historicas)', color='red')

# Graficar las predicciones futuras
plt.plot(fechas_predicciones, future_predictions, label='Predicciones Futuras de Clorofila (hasta mediados de 2025)', color='orange')

# Añadir el umbral de 0.9
plt.axhline(y=0.9, color='green', linestyle='--', label='Umbral de Clorofila = 0.9')

# Configuración de la gráfica
plt.xlabel('Fecha')
plt.ylabel('Concentración de Clorofila')
plt.title('Predicciones de Clorofila: Históricas y Futuras hasta mediados de 2025')
plt.legend()
plt.grid(True)
plt.show()
