...
# Preparar los datos
n_lags = 12
df_supervised = create_supervised_data(Base['Nitratos'].values, n_lags)
...
model = RandomForestRegressor(n_estimators=100, max_depth=20, min_samples_split=2, min_samples_leaf=2, max_features='log2', bootstrap=False)
...
for fecha, pred in zip(fechas_predicciones, future_predictions):
    print(f"Fecha: {fecha.date()}, Predicción de Nitratos: {pred}")

# Visualización de los resultados
plt.figure(figsize=(14, 7))

# Graficar los valores reales y las predicciones para el periodo histórico
plt.plot(dates, true_values, label='Valores Reales de Nitratos', color='green')
plt.plot(dates, predictions, label='Predicciones del Modelo (historicas)', color='orange')

# Graficar las predicciones futuras
plt.plot(fechas_predicciones, future_predictions, label='Predicciones Futuras de Nitratos (hasta finales de 2026)', color='orange')

# Añadir el umbral de 0.9
plt.axhline(y=0.9, color='green', linestyle='--', label='Umbral de Nitratos = 0.9')

# Configuración de la gráfica
plt.xlabel('Fecha')
plt.ylabel('Concentración de Nitratos')
plt.title('Predicciones de Nitratos: Históricas y Futuras hasta finales de 2026')
plt.legend()
plt.grid(True)
plt.show()
