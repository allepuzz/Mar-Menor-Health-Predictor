...
X = Base.drop(['Fecha', 'Fosfatos'], axis=1)
y = Base['Fosfatos']
fecha = Base['Fecha']
...
print(f"Mejores hiperparámetros para Fosfatos: {best_params}")

# Configuración del modelo RandomForest con los mejores hiperparámetros
RF_optimized = RandomForestRegressor(**best_params)

# Configuración de TimeSeriesSplit para validación cruzada temporal con 20 splits
tscv = TimeSeriesSplit(n_splits=255)
...
# Graficar resultados
plot_results('Fosfatos', 'Fosfatos_Real', 'Fosfatos_Predicho', umbral, 'purple', 'brown', 'yellow')
...
plt.title('Importancia de las Características para Fosfatos')
plt.savefig('./Fosfatos_importancia.svg', format='svg')
plt.show()
